...
__global__ void sgemm6(float *A, float *B, float *C, const int M, const int N, const int K)
{
    ...
    __shared__ float shared_A[K_NUM_PER_BLOCK][M_NUM_PER_BLOCK];
    ...
    float a_l_reg[M_NUM_PER_THREAD] = {0.f};
...
#pragma unroll
        for (int i = 0; i < M_NUM_PER_THREAD; ++i)
        {
            ...
            shared_A[tid_x * K_NUM_PER_THREAD + 0][tid_y * M_NUM_PER_THREAD + i] = a_l_reg[0];
            shared_A[tid_x * K_NUM_PER_THREAD + 1][tid_y * M_NUM_PER_THREAD + i] = a_l_reg[1];
            shared_A[tid_x * K_NUM_PER_THREAD + 2][tid_y * M_NUM_PER_THREAD + i] = a_l_reg[2];
            shared_A[tid_x * K_NUM_PER_THREAD + 3][tid_y * M_NUM_PER_THREAD + i] = a_l_reg[3];
        }
#pragma unroll
        ...
        {
            FETCH_FLOAT4(a_reg[0]) = FETCH_FLOAT4(shared_A[k][tid_y * N_NUM_PER_THREAD]);
...
template __global__ void sgemm6<64, 64, 64, 4, 4, 4>(float *A, float *B, float *C, const int M, const int N, const int K);
