...
__global__ void sgemm8(float *A, float *B, float *C, const int M, const int N, const int K)
{
    ...
    float a_reg[2][Y_NUM_PER_THREAD] = {0.f};
    float b_reg[2][X_NUM_PER_THREAD] = {0.f};
    float a_l_reg[4] = {0.f};
    ...
    for (int s = K_NUM_PER_BLOCK; s < K; s += K_NUM_PER_BLOCK)
    {
        ...
        {
            FETCH_FLOAT4(a_reg[compute_stage_idx][0]) = FETCH_FLOAT4(shared_A[compute_stage_idx][k][ctid_y * Y_NUM_PER_THREAD + 0]);
            FETCH_FLOAT4(a_reg[compute_stage_idx][4]) = FETCH_FLOAT4(shared_A[compute_stage_idx][k][ctid_y * Y_NUM_PER_THREAD + 4]);
            FETCH_FLOAT4(b_reg[compute_stage_idx][0]) = FETCH_FLOAT4(shared_B[compute_stage_idx][k][ctid_x * X_NUM_PER_THREAD + 0]);
            FETCH_FLOAT4(b_reg[compute_stage_idx][4]) = FETCH_FLOAT4(shared_B[compute_stage_idx][k][ctid_x * X_NUM_PER_THREAD + 4]);
...
#pragma unroll
                ...
                {
                    sum[ii][jj] += a_reg[compute_stage_idx][ii] * b_reg[compute_stage_idx][jj];
                }
            }
        }
        write_stage_idx ^= 1;
...
template __global__ void sgemm8<128, 128, 8, 8, 8>(float *A, float *B, float *C, const int M, const int N, const int K);
